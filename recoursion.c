...
void	optimizations(t_list **stack_a, t_list **stack, t_list **stack_d, void (*f)(t_list **, t_list **))
{
...
{
	while (min_value(*stack_a) < med && ft_criteria(*stack_a) < ft_lstsize(*stack_a) - 1)
	{
		if (*(int *)(*stack_a)->content < med)
			pb(stack_a, stack);
		else if (ft_lstsize(*stack_a) / 2 - path_to_number(*stack_a, '<', med) > 0)
			ra(stack_a);
		else
			rra(stack_a);
	}
